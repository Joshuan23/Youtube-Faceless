...
# In-memory job tracker: {video_id: "running" | "done" | "error: <msg>"}
_jobs: dict[int, str] = {}
# Pipeline instances for progress access: {video_id: Pipeline}
_pipelines: dict[int, object] = {}
# OAuth flow state
_yt_flow = {}

# ── Startup: reset any in-progress videos to error (their files are gone) ─────
_STUCK_STATUSES = ["scripting", "scripted", "voiced", "thumbnailed", "produced", "uploading"]
try:
    for _stuck in _STUCK_STATUSES:
        for v in db.list_videos(status=_stuck, limit=200):
            db.update_video(
                v["id"],
                status="error",
                last_error="Server restarted — files cleared. Click ↺ Run to rebuild.",
            )
except Exception:
    pass


# ── YouTube status (cached 60s so page loads stay fast) ───────────────────────
_yt_cache: dict = {"value": "unknown", "ts": 0.0}


def _yt_status() -> str:
    now = time.time()
    if now - _yt_cache["ts"] < 60:
        return _yt_cache["value"]
    try:
...
def index():
    ...
    videos       = db.list_videos(limit=25)
...
def generate():
    ...
    def _run():
        try:
            ...
            db.update_video(video_id, status="error", last_error=str(e))
...
def job_status(video_id):
    ...
    progress = (
        pipeline.progress.get(video_id, {"step": "", "pct": 0})
        if pipeline else {"step": "", "pct": 0}
    )
...
def api_retry(video_id):
    ...
    _jobs[video_id] = "running"
    db.update_video(video_id, status="pending", last_error=None)
...
def api_upload(video_id):
    """Upload or rebuild+upload a produced video."""
    video = db.get_video(video_id)
    if not video:
        return jsonify({"ok": False, "error": "Video not found"}), 404
    if _jobs.get(video_id) == "running":
    ...
    _jobs[video_id] = "running"
    db.update_video(video_id, last_error=None)
...
def api_run_all():
    """Run full pipeline for every non-uploaded video, staggered 10s apart."""
    import time as _time
    all_videos = db.list_videos(limit=200)
    queue = []
    for video in all_videos:
        ...
        db.update_video(vid_id, last_error=None)
        queue.append((vid_id, topic, niche))

    def _run_queue(items):
        for idx, (v_id, t, n) in enumerate(items):
            if idx > 0:
                _time.sleep(10)  # stagger to avoid Groq rate limits
            try:
                from src.pipeline import Pipeline
                p = Pipeline(skip_upload=False, dry_run=False, speed_mode=True)
                _pipelines[v_id] = p
                ...
                db.update_video(v_id, status="error", last_error=str(e))
...
def api_test_youtube():
    _yt_cache["ts"] = 0  # force refresh
    result = _yt_status()
    return jsonify({"ok": result == "connected", "status": result})
